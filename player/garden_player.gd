...
func _execute_move(direction: Vector2i) -> bool:
	...
	if not game_map.is_impassable(target_pos.x, target_pos.y):
		# Визуальная анимация поворота и «шажка»
		var sprite: Sprite2D = $Sprite2D
		if direction.x != 0:
			sprite.flip_h = direction.x < 0
		var tween := create_tween()
		tween.tween_property(sprite, "scale", Vector2(1.1, 0.9), 0.08).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
		tween.tween_property(sprite, "scale", Vector2(1, 1), 0.08).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN)
