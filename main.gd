...
func _ready():
    # Гарантируем, что все нужные действия ввода существуют (для быстрого запуска из исходников)
...
# Создает необходимые действия в карте ввода и привязывает клавиши по умолчанию
func _ensure_input_actions():
    # Движение
    _ensure_action("ui_up", KEY_UP)
    _ensure_action("ui_down", KEY_DOWN)
    _ensure_action("ui_left", KEY_LEFT)
    _ensure_action("ui_right", KEY_RIGHT)
    # Действия игрока
    _ensure_action("action_plant", KEY_P)
    _ensure_action("action_water", KEY_W)
    _ensure_action("action_harvest", KEY_H)
    _ensure_action("action_remove", KEY_R)
    _ensure_action("action_interact", KEY_E)
    _ensure_action("toggle_inventory", KEY_I)

func _ensure_action(action_name: String, key_const: int):
    if not InputMap.has_action(action_name):
        InputMap.add_action(action_name)
    # Проверяем, привязана ли уже какая-то клавиша
    var events := InputMap.action_get_events(action_name)
    if events.is_empty():
        var ev := InputEventKey.new()
        ev.physical_keycode = key_const
